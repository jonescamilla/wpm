...
import random
from word_library import words

# generates a new tests based on imported words
def test_maker(test_length):
    new_test = ''

    for _x in range(test_length):
        new_test += words[random.randint(0, len(words) - 1)] + ' '
...
def mock_generator(mock_range, test_length):
